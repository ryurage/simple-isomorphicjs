...
function buildEventBlock($month,$day,$year){
		...
		foreach($result as $rs){
			$id = $rs['event_id'];
			$sql = "SELECT $tableName1.*,$tableName2.event_id 
					FROM $tableName1,$tableName2 
			...
			$aLinkVar .= "<div id=\'divDisabled".$numEvents."\' >".
						 ...
						 "<span class=\'eventLink\'   onclick=renderData(\'divDetails".$numEvents."\',\'{$desc}\');grayOutTheRest(\'".$numEvents."\'); >".$aLink[0]['start_time']." ".$aLink[0]['what']."</span><!-- close span \'eventLink\' -->".
						 "<div id = \'divDetails".$numEvents."\' class=\'details\' style=\'display:none;\' ></div><!-- close \'divDetails\' -->".
...
function buildMonthBlock($month,$day,$year){  // this builds the monthly list of events in the 'month_elist' div
	    ...
	    global $tableName1,$tableName2;
		$sqlDate = $year."-".$month."-".$day;
		$sql = "SELECT $tableName2.event_id FROM $tableName1,$tableName2 WHERE (MONTH($tableName2.date) like MONTH('".$sqlDate."')) AND ($tableName2.id = $tableName1.id)";	
